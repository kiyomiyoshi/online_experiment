/********************************** 
 * Peripheral_Discrimination Test *
 **********************************/


// store info about the experiment session:
let expName = 'peripheral_discrimination';  // from the Builder filename that created this script
...
psychoJS.start({
  ...
  expInfo: expInfo,
  resources: [
    {'name': 's2.png', 'path': 's2.png'},
    {'name': 's1.png', 'path': 's1.png'}
  ]
});
...
async function experimentInit() {
  ...
  text = new visual.TextStim({
    win: psychoJS.window,
    name: 'text',
    text: 'Indicate if grating orientation is upward or downward relative to the horizon.\nKeep looking at the fixation cross during the experiment.\nPress SPACE to start.',
    font: 'Open Sans',
    units: 'height', 
  ...
  trialClock = new util.Clock();
          ...
          condition = list(util.range(0, 60));
  util.shuffle(condition);
...
function trialRoutineBegin(snapshot) {
  return async function () {
    ...
    continueRoutine = true; // until we're told otherwise
    // update component parameters for each repeat
    if (((condition[(trials.thisN - 1)] % 2) === 0)) {
        stim = "s1.png";
        correctAns = "up";
    } else {
        stim = "s2.png";
...
function trialRoutineEachFrame() {
  return async function () {
    ...
    // *image* updates
    if (frameN >= 20 && image.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
    ...
    if (image.status === PsychoJS.Status.STARTED && frameN >= 25) {
